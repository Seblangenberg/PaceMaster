...
exports.cleanupOldEvents = exports.onEventDeleted = exports.onUserCreated = exports.api = void 0;
...
app.put('/api/user/profile', authenticateUser, async (req, res) => {
    try {
        ...
        await db.collection('users').doc(userId).set(Object.assign(Object.assign({}, userData), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }), { merge: true });
...
app.get('/api/events/:eventId', async (req, res) => {
    try {
        ...
        res.json(Object.assign({ id: eventDoc.id }, eventDoc.data()));
    }
    catch (error) {
...
app.post('/api/events', authenticateUser, async (req, res) => {
    try {
        ...
        await eventRef.set(Object.assign(Object.assign({}, eventData), { organizerId: userId, updatedAt: admin.firestore.FieldValue.serverTimestamp(), createdAt: eventData.id ? undefined : admin.firestore.FieldValue.serverTimestamp() }), { merge: true });
        res.json({
            id: eventRef.id,
...
app.delete('/api/events/:eventId', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        if (((_a = eventDoc.data()) === null || _a === void 0 ? void 0 : _a.organizerId) !== userId) {
...
// Get event divisions
app.get('/api/events/:eventId/divisions', async (req, res) => {
    try {
        const { eventId } = req.params;
        const snapshot = await db.collection('events').doc(eventId).collection('divisions').get();
        const divisions = snapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        res.json(divisions);
    }
    catch (error) {
...
app.get('/api/events/:eventId/results', async (req, res) => {
    try {
        ...
        let query = db.collection('events').doc(eventId).collection('results');
        if (divisionId) {
...
app.get('/api/events/:eventId/export', async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${(eventData === null || eventData === void 0 ? void 0 : eventData.name) || 'results'}.csv"`);
        res.send(csvContent);
    }
    catch (error) {
...
app.post('/api/upload/:type/:id', authenticateUser, upload.single('file'), async (req, res) => {
    try {
